...
def multi_task_rg_trainer(train_loader, test_loader, multi_task_model, device, optimizer, scheduler, opt, total_epoch=200):
    ...
    def tracking(grads, y, lambd, it, beta=beta, delta=delta, sigma2=sigma2, sigma3=sigma3, task_loss=None):
        def projection_simplex_sort(v, z=1):
            ...
            return w
        beta_=beta/(1+it)**sigma2
        if "step_wise" in normalize:
            if it < 100:
                beta_=beta/(1+it)**sigma2
            elif 100 <= it <150:
                beta_=0.75*beta/(1+it-100)**sigma2
            else:
                beta_=0.5*beta/(1+it-150)**sigma2
        if "raw_loss" in normalize:
            # print(grads, grads.shape)
            # print(task_loss, task_loss.shape)
            # print("torch.linalg.norm(grads, dim=1)", torch.linalg.norm(grads, dim=0).shape)
            grads = grads/(torch.linalg.norm(grads, dim=0)+1e-8)*task_loss
        #    # print(losses)
        #    grads_ = (grads/torch.linalg.norm(grads, axis=0)).cpu().numpy()*(torch.tensor(losses)/torch.sum(torch.tensor(losses))).cpu().numpy()
        #    y_ =  y -beta/(1+it)**sigma2*(y-grads_)
        #else:
        if "avg_reg" in normalize:
            grads = torch.concat((grads, torch.mean(grads, 1, True)), 1)



        y =  y -beta_*(y-grads)
        # print("grads_", grads_.shape)
        # print("y_", y.shape)
        # print("lambd", lambd.shape)
        # print("y_@lambd)", (y@lambd).shape)
        # print('y', y_.shape)
        # print('y_.T', y_.T.shape)
        # print('y_@lambd', (y_@lambd).shape)
        # lambd_ = projection_simplex_sort(lambd - delta/(1+it)**sigma3*y_.T@(y_@lambd))
        lambd = torch.softmax(lambd - delta/(1+it)**sigma3*y.T@(y@lambd), 0)
        g = y@lambd
    ...
    def overwrite_grad(m, newgrad, grad_dims):
        ...
        for mm in m.shared_modules():
            for param in mm.parameters():
                ...
                # print("grad comp old new dtype", param.grad.dtype, this_grad.dtype)
                # print("grad comp old new device", param.grad.device, this_grad.device)
    ...
    for index in range(total_epoch):
        ...
        for k in range(train_batch):
            ...
            if method == "graddrop":
                ...
                if index == 0:
                    if "avg_reg" in normalize:
                        y = torch.zeros([sum(grad_dims), 3], dtype=torch.float, device=device).detach()
                        lambd = torch.ones([3,1], dtype=torch.float, device=device).detach()/3
                    else:
                        y = torch.zeros([sum(grad_dims), 2], dtype=torch.float, device=device).detach()
                        lambd = torch.ones([2,1], dtype=torch.float, device=device).detach()/2
                for i in range(2):
                    if i == 0:
                        train_loss_tmp[i].backward(retain_graph=True)
                    else:
                    ...
                    grad2vec(multi_task_model, grads, grad_dims, i)
                    multi_task_model.zero_grad_shared_modules()
                g, y, lambd = tracking(grads.detach(), y, lambd, index, task_loss=torch.tensor(train_loss_tmp,dtype=torch.float, device=device).detach())
                overwrite_grad(multi_task_model, g, grad_dims)
                optimizer.step()
