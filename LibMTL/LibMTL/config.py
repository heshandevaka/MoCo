...
_parser.add_argument('--weighting', type=str, default='EW',
    help='loss weighing strategies, option: EW, UW, GradNorm, GLS, RLW, \
        MGDA, PCGrad, GradVac, CAGrad, GradDrop, DWA, IMTL, Tracking')
...
## Tracking
_parser.add_argument('--beta_track', type=float, default=0.1, help='beta for Tracking')
_parser.add_argument('--delta', type=float, default=0.1, help='delta for Tracking')
_parser.add_argument('--sigma2', type=float, default=0.5, help='sigma2 for Tracking')
_parser.add_argument('--sigma3', type=float, default=0.5, help='sigma3 for Tracking')
...
def prepare_args(params):
    ...
    if params.weighting in ['EW', 'UW', 'GradNorm', 'GLS', 'RLW', 'MGDA', 'IMTL',
                            'PCGrad', 'GradVac', 'CAGrad', 'GradDrop', 'DWA', 'DIY', 'Tracking']:
        ...
        elif params.weighting in ['Tracking']:
            if params.beta_track is not None and params.delta is not None and params.sigma2 is not None and params.sigma3 is not None :
                kwargs['weight_args']['beta'] = params.beta_track
                kwargs['weight_args']['delta'] = params.delta
                kwargs['weight_args']['sigma2'] = params.sigma2
                kwargs['weight_args']['sigma3'] = params.sigma3
                # for record aux data, otherwise not needed for optimization
                kwargs['weight_args']['seed'] = params.seed
            else:
                raise ValueError('Tracking needs keywaord beta, delta, sigma2, and sigma3')
    else:
