...
class Trainer(nn.Module):
    ...
    def train(self, train_dataloaders, test_dataloaders, epochs, 
              val_dataloaders=None, large_train_dataloaders=None, return_weight=False):
        ...
        '''
        if large_train_dataloaders is not None:
            print("large_train_dataloaders not None")
            large_train_loader, large_train_batch = self._prepare_dataloaders(large_train_dataloaders)
        train_loader, train_batch = self._prepare_dataloaders(train_dataloaders)
        train_batch = max(train_batch) if self.multi_input else train_batch
        if large_train_dataloaders is not None:
            large_train_batch = max(large_train_batch) if self.multi_input else large_train_batch
        ...
        for epoch in range(epochs):
            ...
            for batch_index in range(train_batch):
                ...
                self.optimizer.step()
            # print(f"Finished training normal batch epoch {epoch}")

            if large_train_dataloaders is not None:
                # print(f"Started computing Jacobian using large batch epoch {epoch}")
                for batch_index in range(large_train_batch):
                    if not self.multi_input:
                        large_train_inputs, large_train_gts = self._process_data(large_train_loader)
                        large_train_preds = self.model(large_train_inputs)
                        large_train_preds = self.process_preds(large_train_preds)
                        large_train_losses = self._compute_loss(large_train_preds, large_train_gts)
                        # self.meter.update(large_train_preds, train_gts)
                    else:
                        large_train_losses = torch.zeros(self.task_num).to(self.device)
                        for tn, task in enumerate(self.task_name):
                            large_train_input, large_train_gt = self._process_data(large_train_loader[task])
                            large_train_pred = self.model(large_train_input, task)
                            large_train_pred = large_train_pred[task]
                            large_train_pred = self.process_preds(large_train_pred, task)
                            large_train_losses[tn] = self._compute_loss(large_train_pred, large_train_gt, task)
                            # self.meter.update(train_pred, train_gt, task)

                    eigmin, kappa = self.model.backward(large_train_losses, large_losses=True, **self.kwargs['weight_args'])
                    break
                # print(f"Finished computing Jacobian using large batch epoch {epoch}")
                print(f"kappa: {kappa} | eigmin: {eigmin}")
